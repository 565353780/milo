...
from base_gs_trainer.Dataset.gs_cameras import GSCameras as Scene
from base_gs_trainer.Method.camera_utils import get_cameras_spatial_extent
...
from regularization.sdf.integration import evaluate_cull_sdf_values
from regularization.sdf.depth_fusion import evaluate_mesh_occupancy
...
from milo.Config.config import PipelineParams
...
def initialize_mesh_regularization(
    ...
    mesh_rasterizer = MeshRasterizer(cameras=scene.train_cameras)
        ...
        if use_delaunay_downsampling:
            if reset_delaunay_samples:
                ...
                if config["radius_culling"] > 0.0:
                    cam_spatial_extent = get_cameras_spatial_extent(scene.train_cameras)
